...
class GPGSAActiveSatellites(BaseModel):
    ...
    def from_nmea(cls, nmea_data: NMEASentence) -> 'GPSSerialGPGSA':
        satilite_prns = [getattr(nmea_data,sat_prn[1]) for sat_prn in nmea_data.fields if sat_prn[1].startswith('sv_id')]
        satilite_prns = [satilite_prn for satilite_prn in satilite_prns if satilite_prn != '']     
...
class SatelliteInfomation(BaseModel):
    ...
    elevation: int = Field(description="The elevation in degrees (0-90) above the horizon")
    azimuth: int = Field(description="Azimuth in degrees (0-359), relative to true north")
    snr: float | None = Field(description="Signal to noise ratio (0-99 dB-Hz). Blank/None if not tracked ")
...
class GPGSVSatellitesInView(BaseModel):
    ...
    """
    expected_satellites: int | None = Field(description="Total number of satellites in veiw", default = None)
    satellites: Dict[int, SatelliteInfomation] = Field(description="The satellite information", default = {})
...
class GPSRedisData(BaseModel):
    ...
    active_prn: List[int] = Field(description="PRN identifiers for the active satellites", default = [])
    hdop: float | str = Field(description="Horizontal dilution of position", default = "")
    
    # Void fix data
    satellites_in_view: Dict[int, SatelliteInfomation] = Field(description="The satellite information", default = {})
